...
object ten {
  ...
  def encodeWithFoldLeft[T](list: List[T]): List[(Int, T)] = {
    nine.pack(list).foldLeft(Nil: List[(Int, T)])((acc, elem) ⇒ (elem.size, elem.head) :: acc).reverse
  }

  def encodeWithFoldRight[T](list: List[T]): List[(Int, T)] = {
    nine.pack(list).foldRight(Nil: List[(Int, T)])((elem, acc) ⇒ (elem.size, elem.head) :: acc)
  }
  ...
  def encode[T](list: List[T]): List[(Int, T)] = {
    ...
    loop(nine.pack(list), Nil)
  }

  def encodeWithMap[T](list: List[T]): List[(Int, T)] = {
    nine.pack(list).map(l ⇒ (l.size, l.head))
  }
